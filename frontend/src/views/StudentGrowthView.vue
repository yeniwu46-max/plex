...
async function loadArchive(force = false) {
  ...
  errorMessage.value = ''
  try {
    const [overviewResult, insights, report, graph, reviewQueue] = await Promise.all([
      ...
      fetchDueMistakeReviews().catch(() => null),
    ])
    ...
    dueReviews.value = reviewQueue?.items ?? []
  } catch (error) {
    if (!overview.value) {
...
async function recordReview(item: StudentMistakeItem) {
  ...
  try {
    ...
    reviewError.value = error instanceof Error ? error.message : '复习结果提交失败'
  } finally {
    ...
    <main class="growth-page">
      ...
      <template v-else>
        ...
        <section class="growth-grid" data-tour="student-learning-report">
          <article class="growth-card review-card">
            <header>
            ...
            <div v-if="dueReviews.length" class="review-list">
              <section v-for="item in dueReviews" :key="item.id" class="review-item">
                <div class="review-item__copy">
                  <strong>{{ item.knowledge_label || item.knowledge_key }}</strong>
                  <p>{{ item.question_title || '重新回忆这道错题的解题要点' }}</p>
                  <small>
            ...
            <div v-if="reviewFeedback" class="review-feedback" role="status">
              <strong>
                图谱节点已更新为“{{ reviewStatusLabel(reviewFeedback.knowledge_graph_update.status) }}”
              </strong>
              <span>下一节点：{{ reviewFeedback.learning_path_update.active_node_id || '按星轨继续' }}</span>
              <small>{{ reviewFeedback.learning_path_update.next_best_action.reason }}</small>
              <small v-if="reviewFeedback.learning_path_update.path_diff">
                路径重规划：{{ reviewFeedback.learning_path_update.path_diff.previous_active_node_id || '无' }}
                → {{ reviewFeedback.learning_path_update.path_diff.current_active_node_id || '无' }}
                <template v-if="reviewFeedback.learning_path_update.path_diff.changed">· 已调整</template>
                <template v-else>· 顺序保持</template>
              </small>
            </div>
          </article>
