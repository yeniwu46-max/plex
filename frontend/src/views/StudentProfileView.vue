...
import { NButton, NInput, NProgress, NTag, useMessage } from 'naive-ui'
...
async function analyze() {
  ...
  loading.value = true
  try {
    const result = await chatDynamicProfile(prompt.value.trim())
...
async function edit(key: ProfileDimensionKey) {
  const value = window.prompt(`修改${labels[key]}`, profile.value?.dimensions[key]?.value || '')
  if (!value?.trim()) return
  profile.value = await updateDynamicProfile({ [key]: value.trim() })
...
<template>
  <DashboardShell active-nav="profile" page-title="学习画像" page-subtitle="对话构建七维动态画像" search-placeholder="" hide-search>
