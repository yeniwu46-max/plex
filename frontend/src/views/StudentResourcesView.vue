...
<template>
  <DashboardShell active-nav="resources" page-title="个性化资源中心" page-subtitle="画像驱动的五类课程资源" search-placeholder="" hide-search>
    <main class="resource-page">
      <section class="generator"><div><h2>生成学习资源包</h2><p>画像解释 → 知识检索 → 教学设计 → 资源生成 → 质量审核 → 路径更新</p><n-tag v-if="aiHealth" :type="aiHealth.effective_backend === 'iflytek_spark' ? 'success' : 'warning'">当前 AI 后端：{{ aiHealth.effective_backend }}</n-tag></div><n-select v-model:value="knowledgeKey" :options="knowledgeOptions" class="knowledge-select" /><n-button type="primary" :loading="generating" @click="generate">生成五类资源</n-button></section>
      ...
      <section class="task-history"><h2>生成任务历史</h2><article v-for="item in taskHistory" :key="item.task_id"><div><strong>{{ item.task_id }}</strong><span>{{ item.status }} · 画像 v{{ item.profile_version }} · {{ item.backend }}</span></div><n-button v-if="item.status === 'failed' && item.recoverable" size="small" @click="retry(item)">重试</n-button></article></section>
...
<style scoped>
.resource-page{width:100%;padding:0 var(--plex-page-gutter-x) 2rem;overflow-y:auto}.generator,.task-panel,.task-history,.resource-card,.resource-detail{border:1px solid rgba(37,245,238,.15);background:rgba(3,16,28,.84);border-radius:16px}.generator{display:flex;align-items:center;gap:1rem;padding:1.2rem}.generator div{flex:1}.generator h2{margin:0;color:#f2fbff}.generator p{margin:.35rem 0 0;color:#7da5b6}.knowledge-select{width:180px}.task-panel{margin-top:1rem;padding:1rem}.task-panel header{display:flex;justify-content:space-between;color:#b9e9e6;margin-bottom:.7rem}.steps{display:flex;flex-wrap:wrap;gap:.5rem;margin-top:.8rem}.processing-note{color:#ffd28a}.task-history{margin-top:1rem;padding:1rem}.task-history h2{margin-top:0;color:#eefbff}.task-history article{display:flex;justify-content:space-between;align-items:center;padding:.65rem 0;border-top:1px solid rgba(37,245,238,.1)}.task-history article div{display:flex;flex-direction:column;gap:.25rem;color:#d8f4f3}.task-history article span{font-size:.78rem;color:#7da5b6}.resource-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;margin-top:1rem}.resource-card{padding:1.1rem;cursor:pointer}.resource-card header,.resource-card footer{display:flex;justify-content:space-between;gap:.5rem}.resource-card h3{color:#f0faff}.resource-card p,.resource-card footer{color:#86a8b7;font-size:.85rem}.resource-detail{margin-top:1rem;padding:1.2rem}.resource-detail header{display:flex;justify-content:space-between}.resource-detail h2,.resource-detail h4{color:#eefbff}.resource-detail pre{white-space:pre-wrap;color:#cce6ef;line-height:1.6;overflow:auto}.resource-detail li{color:#8fb7c4;margin:.45rem 0}@media(max-width:850px){.generator{align-items:stretch;flex-direction:column}.knowledge-select{width:100%}.resource-grid{grid-template-columns:1fr}}
</style>
